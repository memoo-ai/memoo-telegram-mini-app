...
export const requestTwitterFollow = (
  params: RequestTwitterFollowParams,
  showCustomError = true,
): Promise<ApiResponse<boolean>> => {
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
  return http.get(`${prefix}/web-oriented/request-twitter-follow`, {
    params,
    showCustomError,
...
export const uploadDownloadFile = (file: any): Promise<ApiResponse<{ file: string; fileUrl: string }>> => {
  const formData = new FormData();
  formData.set('file', file);
  return http.post(`${prefix}/web-oriented/file-download`, formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
    responseType: 'blob',
...
export const telegramBind = (): Promise<ApiResponse<boolean>> => {
  return http.get(`${prefix}/web-oriented/telegram-bind`, {});
};
export const twitterBind = (
  params: RequestTwitterFollowParams,
  showCustomError = true,
): Promise<ApiResponse<boolean>> => {
  // eslint-disable-next-line @typescript-eslint/consistent-type-assertions
  return http.get(`${prefix}/web-oriented/user-bind-twitter`, {
    params,
    showCustomError,
  } as CustomAxiosRequestConfig);
};
